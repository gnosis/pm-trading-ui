...
describe('Integrations selectors related to network', () => {
  describe('getTargetNetworkId', () => {
    it('Should return network id used in remote provider', () => {
      ...
      expect(getTargetNetworkId(state)).toEqual(4)
    })

    it('Should return undefined if remote provider is not registered', () => {
      const state = generateState({
        providers: Map({}),
      })

      expect(getTargetNetworkId(state)).toBeUndefined()
    })
  })
