...
class MarketShortSellForm extends Component {
  getMaximumReturn(collateralTokenCount, profit) {
    if (
      !isNaN(parseFloat(collateralTokenCount)) &&
      !isNaN(profit) &&
      new Decimal(collateralTokenCount).gt(0) &&
      new Decimal(profit).gte(0)
    ) {
  ...
  getMinProfit(market, outcomeTokenIndex, collateralTokenCount) {
    ...
    if (
      outcomeTokenIndex !== undefined &&
      collateralTokenCount !== undefined &&
      new Decimal(outcomeTokenIndex).gte(0) &&
      new Decimal(collateralTokenCount).gt(0)
    ) {
      const args = {
        outcomeTokenIndex,
  ...
  renderReturnedOutcomeTokens() {
    ...
    if (!isNaN(parseFloat(selectedShortSellOutcome))) {
      // if (event.type === OUTCOME_TYPES.CATEGORICAL) {
      renderedOutcomes = eventDescription.outcomes.filter((outcome, index) => index !== parseFloat(selectedShortSellOutcome))
    }
    ...
    return renderedOutcomes.map((outcome, index) => (
      <div key={index} className="row">
        <div className="entry__color col-md-4" style={{ backgroundColor: colorScheme[index] }} />
        <div className="col-md-8">{outcome}</div>
          ...
          <div className="col-md-12">
            {eventDescription.outcomes.map((label, index) => (
              <Field
                ...
                className={
                  index === parseInt(selectedShortSellOutcome, 10)
                    ? 'marketBuyOutcome selected-outcome'
                    : 'marketBuyOutcome'
                }
                radioValue={index}
                text={label}
  ...
  render() {
    const {
      ...
      market: { event: { type: eventType, collateralToken }, ...market },
    } = this.props
