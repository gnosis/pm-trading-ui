...
import {
  weiToEth, getOutcomeName, isMarketResolved, isMarketClosed, isModerator,
} from 'utils/helpers'
...
const getNewMarkets = (markets = [], limit) => markets.sort((a, b) => a.creationDate < b.creationDate).slice(0, limit || markets.length)

const getSoonClosingMarkets = (markets = [], limit) => markets
    ...
    return markets.map((market) => {
      ...
      return (
        <div className="dashboardMarket dashboardMarket--new" key={market.address} onClick={viewMarket}>
          <div className="dashboardMarket__title">
...
&nbsp;
      ...
      return (
        <div className="dashboardMarket dashboardMarket--onDark" key={index} onClick={viewMarket}>
          ...
          <div className="outcome row">
            ...
            <div className="col-md-2 dashboardMarket--highlight">
              {TRANSACTION_DESCRIPTIONS[trade.orderType]}
            </div>
          </div>
        </div>
      )
  ...
  renderWidget(marketType) {
    ...
    const whitelistedMarkets = markets.filter(market => Object.keys(market).length
        && market.oracle
        && market.event
    ...
    if (marketType === 'newMarkets') {
      return (
        <div className="dashboardWidget col-md-6" key={marketType}>
          <div className="dashboardWidget__market-title">
Latest Markets
          </div>
          <div
            className={cn({
