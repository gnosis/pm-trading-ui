...
const FormRadioButton = ({ input, radioValue, text, className, highlightColor }) => (
  <div className={`radioButton ${className || ''}`}>
    <input
      type="radio"
      className={`radioButton__input ${className ? `${className}__input` : ''}`}
      ...
      value={radioValue}
    />
    <label className={`radioButton__text ${className ? `${className}__text` : ''}`} htmlFor={`radioButton_${input.name}_${radioValue}`}>
      {text}
    </label>
  </div>
)
