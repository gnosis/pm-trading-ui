...
const AddressSection = ({ hasRegistered, mainnetAddress, openSetMainnetAddressModal }) => (
  <Block className={cx('addressSection')}>
    ...
    <Paragraph className={cx('address')}>
      {hasRegistered ? mainnetAddress : 'No address specified yet.'}
    </Paragraph>
    {!hasRegistered && (
      <Link to="/scoreboard" href="/scoreboard" onClick={openSetMainnetAddressModal}>Setup claim address</Link>
    )}
  </Block>
)
